...
**token 成本**:检查 = 纯 shell(零);刷新 = 纯 shell(零);唯一花 token 的是刷新后重做 rule 7
(见下),而这**只发生在真正改了的那几篇**。所以「100 篇 reference、这周只改了 1 篇」≈ 只有那 1 篇
进 agent 上下文。

> [!warning] 刷新会**整体覆盖正文**,之前对快照做的任何本地加工都会被还原
> `refresh_reference.sh` 保留的只有 **frontmatter**(这是设计如此);正文是从源文件重新取的,
> 所以上一次按 rule 7 把相对链接降级/改成 `[[slug]]` 的成果**全部丢失**——不是"新正文引入了
> 新链接"才丢,是**原本就有的那些也会回来**。
>
> 因此 **rule 7 必须在每次 refresh 之后无条件重做一遍**,不能靠"这次源文档改的地方跟链接无关"
> 来跳过。判断方法是直接扫,不是猜:
> ```bash
> # 列出所有还带相对 md 链接的快照(输出为空才算干净)
> grep -rlP '\]\((?!https?://)[^)]+\.md[^)]*\)' $KNOWLEDGE_VAULT/reference/
> ```
> 实测踩过:一次刷新 4 篇,其中 2 篇的相对链接全数复活(6 条 + 2 条),而源文档那次改动跟链接
> 毫无关系。
